...
- Local install registers `Hushward-NightWake` in Task Scheduler at `00:30` with `WakeToRun`, 5 minute repetition, 6 hour duration, and `--scheduled-check`, plus the existing Run key.
